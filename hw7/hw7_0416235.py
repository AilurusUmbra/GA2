...
def ev3(cfg):
    ...
    if cfg.evaluator == 'particles1d':
        workerFunc = Particles1D.fitnessFunc
        ...
        workerFunc = Rastrigrin.fitnessFunc
    ...
    for i in range(cfg.generationCount):
        ...
        p = Pool(processes=cfg.numProcesses)
        states = [child.state for child in offspring.population]
        fitnesses = p.map(workerFunc, states)
        for idx in range(len(offspring.population)):
            offspring.population[idx].fit = fitnesses[idx]
        ...
        _best = printStats(population,i+1)

        # plot metrics
        pausetime = 0.00001
        ax[cfg.numProcesses-1].set_title(str(cfg.numProcesses)+' processes')
        plt.ion()
        plt.show()
        plt.pause(pausetime)
        ax[cfg.numProcesses-1].scatter(i, _best, c='b')
        ax[cfg.numProcesses-1].legend(['best_fitness'])
...
def main(argv=None):
    ...
    try:
        ...
        time_taken = []
        global fig, ax
        fig, ax = plt.subplots(1, cfg.numProcesses, figsize=(cfg.numProcesses*3,4))

        for n in range(cfg.numProcesses):
            # run with 1~5 processes
            cfg.numProcesses = n+1
            ...
            time_taken.append(round(end-start, 3))

        print(time_taken)

        # plot time_taken
        fig, ax = plt.subplots(figsize=(4,4))
        plt.title("elasped time")
        plt.xlabel('Number of processes')
        plt.ylabel('Time')
        plt.plot(range(1,cfg.numProcesses+1), time_taken)
        plt.plot(range(1,cfg.numProcesses+1), time_taken, 'o')
        plt.show()
        ...
        print("Num of Processes:", cfg.numProcesses)
