#
# 0416235.py: An elitist (mu+mu) generational-with-overlap EA
#
#
# To run: python 0416235.py --input ev3_example.cfg
#         python 0416235.py --input my_params.cfg
#
...
class EV3_Config:
    ...
    options={'populationSize': (int,True),
             ...
             'rastrigrin_A': (float,False),
             'rastrigrin_N': (int,False),
...
def ev3(cfg):
    ...
    if cfg.fitness == 'lattice':
        Lattice.selfEnergy=cfg.selfEnergy
        Lattice.interactionEnergy=cfg.interactionEnergy
        IntVectorIndividual.fitFunc=Lattice.fitnessFunc
    ...
    elif cfg.fitness == 'rastrigrin':
        Rastrigrin.A=cfg.rastrigrin_A
        Rastrigrin.nVars=cfg.rastrigrin_N
        ...
        MultivariateIndividual.nLength=cfg.rastrigrin_N
        MultivariateIndividual.learningRate=1.0/math.sqrt(cfg.rastrigrin_N)
        Population.individualType=MultivariateIndividual
    else:
        raise Exception('Unknown fitness type: ' + str(cfg.fitness))
    ...
    for i in range(cfg.generationCount):
        ...
        # plot metrics
        pausetime=0.1
        ax.set_title(cfg.fitness)
        plt.ion()
        plt.show()
        plt.pause(pausetime)
        ax.scatter(i, _best, c='b')
        ax.legend(['best_fitness'])
